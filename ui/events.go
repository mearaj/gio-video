...
func (p *Player) handleUIEvents(gtx C) {
	...
	if p.screenClickable.Clicked(gtx) {
	...
	if p.incrementButton.Clicked(gtx) {
		now := time.Now().UnixMilli()
	...
	if p.decrementButton.Clicked(gtx) {
		now := time.Now().UnixMilli()
